...
- 🟢 **CERRADO (por diseño)** — `MwtJWTAuthentication` relee `users.mwtuser` por request a propósito (pool de entidades SIEMPRE fresco, fix documentado del bug de asignaciones tardías). Es UN lookup por PK indexado por request — costo O(1) aceptado conscientemente; reevaluar solo si aparece en el top de pg_stat_statements.
