...
// Fable5 · WAVE D ("partir archivos gigantes"): las cards financieras
// (OCPagosCard / OCTransferCostsCard, ~270 líneas) viven ahora en su
// propio módulo. Se re-exportan para compatibilidad con imports viejos.
import { OCPagosCard, OCTransferCostsCard } from "../components/expedientes/OCFinanceCards.jsx";
export { OCPagosCard, OCTransferCostsCard };
import {
      ...
      .then(async (o) => {
        ...
        if (o.client_id && lineasArr.length > 0) {
          ...
          if (uniquePidIds.length > 0) {
            try {
              // Fable5 · WAVE D: UN request batch (?ids=) en vez de un
              // GET por producto único de la OC.
              const prodResp = await productosApi
                .list({ ids: uniquePidIds.join(",") })
                .catch(() => []);
                  ...
                  <div style={{flex: 1, minWidth: 0}}>
                    <div className="flex ai-center gap-2" style={{minWidth: 0}}>
                      ...
                      <span className="body-sm" style={{fontWeight: 600, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}
                            title={e.codigo || eid}>{e.codigo || eid}</span>
...
// (Fable5 · WAVE D: OCPagosCard extraída a OCFinanceCards.jsx)


// (Fable5 · WAVE D: OCTransferCostsCard extraída a OCFinanceCards.jsx)
